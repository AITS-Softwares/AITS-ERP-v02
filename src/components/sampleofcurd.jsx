...
import { getGSTINValidationMessage, isValidGSTIN, normalizeGSTIN } from "@/utils/gstin";
...
2: (d) => {
  ...
  4: (d) => {
    ...
    if (d.gstNumber && !isValidGSTIN(d.gstNumber))
      e.gstNumber = getGSTINValidationMessage(d.gstNumber);
    return e;
  },
  5: () => ({}),
  6: () => ({}),
...
const fetchCustomers = useCallback(async () => {
  ...
  const clearGSTMappedFields = (current) => ({
    ...current,
    gstLegalName: "",
    gstTradeName: "",
    gstStatus: "",
    ...
    billingAddresses: [
      {
        ...(current.billingAddresses?.[0] || EMPTY_ADDR),
        address1: "",
        address2: "",
        city: "",
        state: "",
      ...
      ...(current.billingAddresses || []).slice(1),
    ],
  });
  ...
  const handleChange = (e) => {
    ...
    setCd((p) => {
      if (name === "gstNumber" && normalizeGSTIN(value) !== normalizeGSTIN(p.gstNumber)) {
        return { ...clearGSTMappedFields(p), gstNumber: value };
      }
      return { ...p, [name]: value };
    });
    clearErr(name);
  ...
  const handleFetchGSTDetails = async () => {
    const gstin = normalizeGSTIN(cd.gstNumber);
    const validationMessage = getGSTINValidationMessage(gstin);
    if (validationMessage) {
      setErrs((p) => ({ ...p, gstNumber: validationMessage }));
    ...
    try {
      ...
      const res = await axios.post(
        "/api/integrations/erpnext/gst/lookup",
        { gstin },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      setCd((p) => ({
        ...
        customerName: data.tradeName || data.legalName || "",
        pan: data.pan || gstin.slice(2, 12),
        gstCategory: data.gstCategory || (data.status?.toLowerCase() === "active" ? "Registered Regular" : ""),
        emailId: data.email || "",
        mobileNumber: data.mobile || "",
        contactPersonName: data.contactPerson || "",
        billingAddresses: [
          {
            ...(p.billingAddresses?.[0] || EMPTY_ADDR),
            address1: data.addressLine1 || data.address || "",
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-semibold disabled:opacity-60 flex items-center gap-2"
                  >
                    {gstLookupLoading ? <FaSpinner className="animate-spin" /> : <FaSearch />}
                    Fetch
                  </button>
                </div>
                <Err k="gstNumber" />
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              ...
              <RRow label="Legal Name" value={cd.gstLegalName} />
              <RRow label="Trade Name" value={cd.gstTradeName} />
              <RRow label="GST Status" value={cd.gstStatus} />
